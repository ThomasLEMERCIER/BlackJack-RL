...
class InfiniteSimpleBlackjack(gym.Env):
    """ Infinite Simple Blackjack environment
...
class SimpleBlackjack(gym.Env):
    ...
    def step(self, action: int) -> tuple[np.ndarray, float, bool, bool, dict]:
        ...
        if action == 1:  # Hit
            self.player_hand.add_card(self.draw_card())
            self.current_state = np.array([self.player_hand.value, self.dealer_hand.cards[0], self.player_hand.aces, len(self.player_hand)], dtype=np.int32)
            if self.player_hand.value > 21:
                return self.current_state, -1, True, False, {}
            else:
                return self.current_state, 0, False, False, {}
    ...
    def render(self, mode: str="ansi") -> None:
        ...
        if mode == "hidden":
            print(f"Player's hand: {self.player_hand}")
            print(f"Dealer's hand: {self.dealer_hand.cards[0]} ?")
            print(f"Cards in hand: {len(self.player_hand)}")
