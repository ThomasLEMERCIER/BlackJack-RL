...
from src.utils.data_struct import Transition, SarsaParameters

def play_episode(env: gym.Env, agent: SarsaAgent, render: bool = False):
    state = env.reset()
    if render:
        env.render()
...
def main(env: gym.Env, agent: SarsaAgent, n_episodes: int):
    start = time.time()
    n_wins = 0
    n_draws = 0
